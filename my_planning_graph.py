...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        #for state_node in self.s_levels[level-1]:
        #    for action_node in state_node.parents:
        #        action_nodes.add(state_node)
        #        action_node.parents.add(state_node)
        #        action_node.children.add(state_node)
        #self.s_levels.append(action_nodes)
        ...
        #    x = PgNode_a(action)
        #    if x.prenodes.issubset(self.s_levels[level]):
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if (all (x in node_a2.action.effect_rem for x in node_a1.action.effect_add)) and (all (x in node_a1.action.effect_rem for x in node_a2.action.effect_add)):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if (any (x in node_a2.action.precond_neg for x in node_a1.action.effect_add)) \
                or (any (x in node_a1.action.precond_neg for x in node_a2.action.effect_add) \
            or any (x in node_a2.action.precond_pos for x in node_a1.action.effect_rem)) \
                        or (any (x in node_a1.action.precond_pos for x in node_a2.action.effect_rem)):
            return True
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            print("current goal is: " + g)
            goal_node = PgNode_s(g,True)
            for x in range(len(self.s_levels)):
                symbols=[]
                for s in self.s_levels[x]:
                    symbols.append(s)


                if goal_node in symbols:
                    level_sum += x
                    break
